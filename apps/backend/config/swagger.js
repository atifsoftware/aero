...
// Metadata options for OpenAPI spec (Aero Core)
const baseOptions = {
  definition: {
    openapi: '3.0.0',
    info: {
      ...
      version: '1.0.0',
      description: 'Interactive RESTful API Explorer for Aero, Test endpoints in real-time by authorizing with a JWT or Personal Access Token.',
      contact: {
        name: 'Aero Core Team'
      }
    },
...
function generateAutoSwaggerSpec() {
  ...
  try {
    ...
    try { apiRouter = require('../../routes/api'); } catch (err) { }
  }
  try {
    authRouter = require('../routes/auth');
  } catch (e) {
    try { authRouter = require('../../routes/auth'); } catch (err) { }
  }
